...
#ifndef _H_MY_LINETRASE_
#define _H_MY_LINETRASE_

void linetrase_pd(ports_t *linetrase, int speed, float p_gain,float d_gain);
...
void linetrase_pd_log(ports_t *linetrase, int speed, float p_gain,float d_gain,int *x)
{
    ...
    ev3_lcd_draw_line((uint32_t)(*x / 2.0),(int32_t)final_steering + 70,(uint32_t)(*x / 2.0 + 1),(int32_t)final_steering + 70);

}

void linetrase_pd_cm(ports_t *linetrase,int speed,float p_gain,float d_gain,float cm,end_situation_t end)
{
...
uint8_t linetrase_pd_for(ports_t *linetrase, int speed, float p_gain,float d_gain,int to,int right_black,int left_black)
{
    ...
    steering_start(linetrase, speed, (int)final_steering);
    //動作開始
    ...
    //過去配列に格納、インデックス更新
    sorts_linetrase_for_write(linetrase,abs(left_color - right_color));
    ...
    {
        if (to == _H_FOR_RIGHT_ && right_color < right_black && abs(linetrase -> ave) < _H_STER_LIMIT_)
        {
            finish = 1;
        }
        else if (to == _H_FOR_LEFT_ && left_color < left_black && abs(linetrase -> ave) < _H_STER_LIMIT_)
        {
            finish = 1;
        }
        else if (to == _H_FOR_DOUBLE_ && left_color < left_black && right_color < right_black && abs(linetrase -> ave) < _H_STER_LIMIT_)
        {
            finish = 1;
        }
    }
