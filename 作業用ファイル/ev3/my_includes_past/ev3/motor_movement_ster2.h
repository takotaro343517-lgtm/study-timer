...
//見てくれよ、俺のpro開始を
void mv__motors_start_pro_ver2(ports_t *motors,int power,float pro)
{
    ev3_motor_set_power(motors -> right_motor_port,0);
    ev3_motor_set_power(motors ->  left_motor_port,0);

    float now_power;
    while(now_power == power)
    {
        ev3_motor_set_power(motors -> right_motor_port, now_power * motors -> right_motor_fix);
        ev3_motor_set_power(motors -> left_motor_port, now_power * motors -> left_motor_fix);
        now_power = now_power + pro;
        tslp_tsk(10);
    }
...
//見てくれよ、俺のpro停止を
void mv__motors_stop_pro_ver2(ports_t *motors, float pro)
{
    int fst_right = ev3_motor_get_power(motors -> right_motor_port);
    int fst_left = ev3_motor_get_power(motors -> left_motor_port);
    int fst_power;
    if(fst_right >= fst_left)
    {
        fst_power = fst_right;
    }else
    {
    ...
    ev3_motor_set_power(motors -> right_motor_port, fst_power);
    ev3_motor_set_power(motors -> left_motor_port, fst_power);
    float now_power;
    while (now_power < 0.10)
    {
        now_power = fst_power - pro;
        ev3_motor_set_power(motors -> right_motor_port, now_power);
        ev3_motor_set_power(motors -> left_motor_port, now_power);
        pro = pro + pro;
        tslp_tsk(10);
    }
...
mv__motors_run_cm_ver2(ports_t *motors, int power,float cm, start_situation_t start,end_situation_t stop)
{
    int first_right = ev3_motor_get_counts(motors -> right_motor_port);
    int first_left = ev3_motor_get_counts(motors -> left_motor_port);
    int goal = cm * (motors -> dis_convert_val);
    ...
    {
        mv__motors_start_pro_ver2(motors,power,1.0);
    }
    int ave;
    int change = (int)(goal * _START_SIN_);
    if (stop == SIN_END || stop == PRO_END)
